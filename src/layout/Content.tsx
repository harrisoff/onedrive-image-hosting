...
export default () => {
  ...
  useEffect(() => {
    if (!editMode) {
      setItemList((prevItemList) => prevItemList.map((i) => ({ ...i, selected: false })))
    }
  }, [editMode])
  ...
  const defaultFolderList: {
    ...
    () =>
      Array.from(new Set([...uploadHistory.map((i) => i.folder), defaultFolder])).map((label) => ({
        label,
        selected: label === defaultFolder,
      })),
    [uploadHistory],
  )
  const [folderList, setFolderList] = useState(defaultFolderList)
  const selectedFolder = useMemo(() => folderList.find((i) => i.selected)?.label || defaultFolder, [folderList])
  ...
  const { error, errorDescription } = useMemo(() => {
    if (hashParams.error) {
      return hashParams
    }
    if (!hashParams.accessToken) {
      return {
        error: (
          <>
            Please <a onClick={openPopup}>get token</a>
          </>
        ),
            ...
            {showAdvanced ? (
              <Space>
                ...
                <FolderSelector
                  ...
                  onAdd={(folder) => {
                    setFolderList((prev) =>
                      Array.from(new Set([...prev.map((i) => i.label), folder])).map((i) => {
                ...
                />
                <span>
                  <Input value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder="Search filename" />
                </span>
                {editMode ? (
                  <>
                    <Popconfirm
                  ...
                  <Tooltip
                    ...
                    title={
                      <>
                        <span>1. Latest {uploadHistoryLength} items will be saved to browser cache.</span>
                        <br />
